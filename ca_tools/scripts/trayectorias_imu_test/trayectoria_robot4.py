...
class MoverRobot():
    ...
    def _corregir_angulo(self, angulo):
        if angulo < 0:
    ...
    def leer_odometria(self, msg):
        ...
        #self.theta = self._corregir_angulo(self.theta)
    ...
    def mover(self, punto):
        ...
        while abs(inc_x) > 0.05  or abs(inc_y) > 0.05:  
            ...
            if abs(angulo_meta - self.theta) > 0.2:
                if angulo_meta - self.theta < 0:
                    if abs(angulo_meta - self.theta) > math.pi:
                        vel.linear.x = 0.0
                        vel.angular.z = 0.3
                    else:
                        vel.linear.x = 0.0
            ...
            angulo_meta = self._corregir_angulo(atan2(inc_y, inc_x))
            #print (self.theta)
            #print (angulo_meta)
